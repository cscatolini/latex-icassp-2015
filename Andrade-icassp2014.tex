...
\item Copy the previous pitch template: the template of a given pitch $i_n$ is repeated for all subsequent pitchs $i \in [i_{n+1}, i_{p-1}]$ until another template is found (pitch $i_p$);
\item Interpolate neighbour pitchs' templates: the template for the pitchs $i \in [i_{n+1}, i_{p-1}]$ are obtained by linearly interpolating the templates of the pitchs $i_n$ and $i_p$ (on a dB scale).
\end{enumerate}
...
We suggest a modification of the BHAD model where the convergence rate coefficient $\beta_{brake}(n)$ depends on the pitch, so we can apply a stronger brake for the templates of the pitchs which were actually learned:
